...
class InSarDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.image_size = self.config.dataloader.image_size
        self.seed = self.config.dataloader.seed
        self.timeseries_length = self.config.dataloader.timeseries_length
        ...
        self.atmospheric_channels = self.config.dataloader.atmospheric_channels
        self.geomorphology_channels = self.config.dataloader.geomorphology_channels
        self.annotation_dir = self.config.dataloader.annotation_path
        self.zarr_path = self.config.dataloader.zarr_path

        # Load timeseries data
        self.timeseries_dict = json.load(open(self.config.dataloader.timeseries_data, "r"))
        # Load annotations
        self.complete_annotation_dict = json.load(open(self.config.dataloader.annotation_path, "r"))

        # Process mode end and start dates for temporal split
        if self.mode == 'train':
            self.mode_start = datetime.strptime(str(self.config.dataloader.train_years[0]), "%Y%m%d")
            self.mode_end = datetime.strptime(str(self.config.dataloader.train_years[1]), "%Y%m%d")
        elif self.mode == 'val':
            self.mode_start = datetime.strptime(str(self.config.dataloader.val_years[0]), "%Y%m%d")
            self.mode_end = datetime.strptime(str(self.config.dataloader.val_years[1]), "%Y%m%d")
        else:
            self.mode_start = datetime.strptime(str(self.config.dataloader.test_years[0]), "%Y%m%d")
            self.mode_end = datetime.strptime(str(self.config.dataloader.test_years[1]), "%Y%m%d")
        ...
        self.augmentations = (
            augmentations.get_augmentations(config, config.dataloader.image_size)
            if config.dataloader.augment
            else None
        )
        ...
        if not config.dataloader.webdataset:
    ...
    def parse_timeseries(self, sample):
        ...
        insar_tensor = self.crop_around_deformation(insar=insar_tensor, crop_size=self.image_size, target_mask_method=self.config.dataloader.mask_target, annotation=sample)
    ...
    def _prepare_model_input(self, insar_tensor):
        if "unet" or "deeplab" or "vit" in self.config.model.architecture.lower():
            ...
            if self.webdataset_write == False:
                if self.config.dataloader.timeseries_length != 1:
                    if self.config.dataloader.mask_target == "peak":
                        counts = torch.sum(prep_mask, dim=(1, 2))
                        prep_mask = prep_mask[torch.argmax(counts), :, :, :]
                    elif self.config.dataloader.mask_target == "union":
                        prep_mask = torch.sum(prep_mask, dim=0)
                        prep_mask = torch.where(prep_mask > 0, 1, 0)
                    elif self.config.dataloader.mask_target == "last":
            ...
            raise NotImplementedError(
                "Model {} not implemented for dataloader output".format(self.config.model.architecture)
            )
    ...
    def __getitem__(self, index):
        ...
        if (
            ...
            or self.config.dataloader.webdataset is True
        ):
